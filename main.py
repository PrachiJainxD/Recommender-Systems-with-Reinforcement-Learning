...
# mixture_model = MixtureModel(path='dataset', alpha=0.75, beta_weight=0.25, k=K_LAST_PURCHASES, verbose=True, save_path='saved_models/'+MONTE_CARLO_MODEL_PATH)
...
for i in range(K_LAST_PURCHASES):
    mm = MDP(PATH, k=i+1, alpha=0.75, beta_weight=0.25, save_path=SARSA_MODEL_PATH)
    mm.initialise_mdp()
    mm.sarsa_for_optimal_policies(N=100)

# for i in range(K_LAST_PURCHASES):
#     mm = MDP(PATH, k=i+1, alpha=0.75, beta_weight=0.25, save_path=TD_MODEL_PATH)
#     mm.initialise_mdp()
